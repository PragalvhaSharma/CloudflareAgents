  ...
  execute: async ({ city }) => {
    try {
      ...
      const weatherDescription =
        weatherDescriptions[current.weather_code] || "Unknown conditions";
...
const getLocalTime = tool({
  ...
  inputSchema: z.object({
    location: z
      .string()
      .describe("The city, country, or timezone to get the time for")
  }),
  execute: async ({ location }) => {
    try {
      ...
      let matchingTimezone = timezones.find((tz) => {
        ...
        return (
          tzLower.includes(normalizedLocation) ||
          normalizedLocation.includes(
            tzLower.split("/")[1]?.toLowerCase() || ""
          ) ||
      ...
      const timeResponse = await fetch(
        `https://worldtimeapi.org/api/timezone/${matchingTimezone}`
      );
...
const getRandomFact = tool({
  description:
    "Get a random interesting fact about science, history, nature, or general knowledge",
  inputSchema: z.object({
    category: z
      .enum(["random", "science", "history", "nature"])
      .optional()
...
const getNasaAPOD = tool({
  description:
    "Get NASA's Astronomy Picture of the Day - a daily space photo with scientific explanation",
  inputSchema: z.object({
    date: z
      .string()
      .optional()
      .describe(
        "Optional date in YYYY-MM-DD format. If not provided, returns today's picture"
      )
  }),
  execute: async ({ date }) => {
    try {
...
const getStockData = tool({
  description:
    "Get real-time stock market data for any publicly traded company using Yahoo Finance API",
  inputSchema: z.object({
    symbol: z
      .string()
      .describe("Stock ticker symbol (e.g., AAPL, GOOGL, MSFT)"),
    interval: z
      .enum(["1min", "5min", "15min", "30min", "1h", "1day", "1week", "1month"])
      .optional()
...
const getCountryInfo = tool({
  description:
    "Get detailed information about any country including population, currency, flags, and more",
  inputSchema: z.object({
